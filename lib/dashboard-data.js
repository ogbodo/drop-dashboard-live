...
// A deleted rider/driver keeps its profile row (nine foreign keys point at it, so the
// row cannot be removed) but both delete paths stamp deactivated_at. Spread this into
// any browse list or headline count so deleted accounts drop off the dashboard, while
// their history still resolves by id where it is looked up directly (rides, referrals,
// the Archived view).
const EXCLUDE_DELETED = { deactivated_at: "is.null" };

export const getOverviewData = async (admin, config) => {
  const [driverCount, verifiedDriverCount, paidDriverCount, onlineDriverCount] =
    await Promise.all([
      admin.count("profiles", { ...EXCLUDE_DELETED, role: "eq.driver" }),
      admin.count("profiles", { ...EXCLUDE_DELETED, role: "eq.driver", is_verified: "eq.true" }),
      admin.count("profiles", { ...EXCLUDE_DELETED, role: "eq.driver", has_paid: "eq.true" }),
      admin.count("profiles", { ...EXCLUDE_DELETED, role: "eq.driver", is_online: "eq.true" }),
    ]);
  ...
  const [
    ...
    admin.count("profiles", { ...EXCLUDE_DELETED, role: "eq.customer" }),
...
export const getDriversData = async (admin, filters = {}, options = {}) => {
  ...
  const [
    ...
    driverIds.length
      ? safeSelect(() =>
          admin.select("driver_flags", {
            driver_id: inFilter(driverIds),
            resolved_at: "is.null",
            order: "created_at.desc",
            select:
              "id,driver_id,reason,source_review_id,created_by_username,created_at",
          }),
        )
      : Promise.resolve([]),
  ]);
  ...
  const flagsByDriverId = groupBy(driverFlags, "driver_id");
  ...
  const rows = drivers
    .map((driver) => {
      ...
      const nextDriver = {
        ...driver,
        activation_state: buildDriverActivationState(driver),
        flags: flagsByDriverId.get(driver.id) || [],
...
// Raise an internal flag on a driver, from a low-rating review or their detail
// panel. The reason is required so a flag is never a bare mark with no context.
export const flagDriver = async (admin, session, payload = {}) => {
  ...
  if (!reason) {
    throw new Error("A reason for the flag is required.");
  }

  const [row] = await admin.insert("driver_flags", {
    created_by_account_id: session?.accountId || null,
    created_by_username: session?.displayName || session?.username || "system",
    driver_id: driverId,
    reason,
    source_review_id: payload.sourceReviewId || null,
  });

  return row || null;
};

// Close an open flag once ops has dealt with it. Filtered on resolved_at is null
// so resolving the same flag twice is a no-op rather than overwriting the record.
export const resolveDriverFlag = async (admin, session, payload = {}) => {
  const flagId = String(payload.flagId || "").trim();

  if (!flagId) {
    throw new Error("A flag is required.");
  }
  ...
  const rows = await admin.update(
    "driver_flags",
    {
      ...
      resolved_by_username: session?.displayName || session?.username || "system",
      resolved_note: payload.note ? String(payload.note) : null,
    },
    { id: `eq.${flagId}`, resolved_at: "is.null" },
  );

  return rows?.[0] || null;
};

// Archive a rider or driver account and FREE its phone/email for reuse. Snapshots
// the profile into archived_accounts (source 'admin', freed true), then anonymises
// the live profile and nulls its phone and email so the same details can register
// again. The row stays for the trip/payment history that references it; the edge
// function that calls this also frees the number in Auth. Returns the phone/email
// it freed so the caller can clear them from auth.users too.
export const archiveAndFreeProfile = async (admin, session, payload = {}) => {
  ...
  // Preserve everything, not just the profile. The vehicle carries photos this
  // delete is about to strip, so they are captured here before anonymising; the
  // payout accounts round out a self-contained record. Both are empty for a rider.
  const [vehicles, payoutAccounts] = await Promise.all([
    safeSelect(() =>
      admin.select("vehicles", { driver_id: `eq.${userId}`, select: "*" }),
    ),
    safeSelect(() =>
      admin.select("driver_payout_accounts", {
        driver_id: `eq.${userId}`,
  ...
  await admin.insert("archived_accounts", {
    deleted_by: session?.displayName || session?.username || "admin",
    email: profile.email ?? null,
    freed: true,
    full_name: profile.full_name ?? null,
    original_user_id: userId,
    phone: profile.phone ?? null,
    profile_snapshot: { payout_accounts: payoutAccounts, profile, vehicles },
  ...
  await admin.update(
    "profiles",
    {
      ...
      license_selfie_url: null,
      nin_number: null,
      // Phone nulled, not kept (email nulled above): an admin delete FREES both
      // for reuse, the opposite of a self-delete which keeps them locked.
      phone: null,
      updated_at: now,
    },
    { id: `eq.${userId}` },
  ...
  try {
    await admin.update(
      "vehicles",
      { is_active: false, registration_photo_url: null, vehicle_image_url: null },
      { driver_id: `eq.${userId}` },
    );
  } catch (vehicleError) {
    // Non-fatal: the account is already archived and anonymised.
  }

  return { email: profile.email ?? null, phone: profile.phone ?? null, userId };
};

// The archive, newest first, for the dashboard to show and look up. Wrapped in
// safeSelect: the drop-backend migration that creates archived_accounts and this
// dashboard deploy separately, so during the window before the table exists the
// section degrades to empty rather than throwing.
export const getArchivedAccounts = async (admin, filters = {}) => {
  const rows = await safeSelect(() =>
    admin.select("archived_accounts", {
      limit: Math.min(Number(filters.limit) || 100, 300),
      order: "archived_at.desc",
      select:
        "id,original_user_id,role,full_name,phone,email,source,freed,deleted_by,reason,archived_at",
    }),
  );
  ...
  const filtered = search
    ? rows.filter((row) =>
        [row.full_name, row.phone, row.email, row.deleted_by].some((value) =>
          String(value || "").toLowerCase().includes(search),
        ),
      )
    : rows;

  return { accounts: filtered, total: rows.length };
};
