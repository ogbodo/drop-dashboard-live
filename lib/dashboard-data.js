  ...
  const [accounts, partners] = await Promise.all([
    safeSelect(() =>
      admin.select("dashboard_accounts", {
        order: "created_at.desc",
        select:
          "id,username,display_name,role,partner_id,is_active,last_login_at,password_updated_at,created_at,updated_at,created_by",
      }),
    ),
    safeSelect(() =>
      admin.select("partners", {
...
export const getPartnerWorkspaceData = async (admin, { partnerId } = {}) => {
  ...
  const [partner, links, payouts, commissions, codes, rides, account] = await Promise.all([
    safeSelectOne(() =>
