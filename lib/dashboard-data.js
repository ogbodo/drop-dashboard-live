...
const isLeadershipViewer = (viewerRole) => TEAM_LEADERSHIP_ROLES.has(viewerRole);
...
const maskRideFinancialsForStaff = (entry) =>
  entry
    ? {
        ...entry,
...
const maskDriverForStaff = (driver) =>
  driver
    ? {
        ...driver,
...
const maskCustomerForStaff = (customer) =>
  customer
    ? {
        ...customer,
        dob: null,
...
export const getLiveOpsData = async (admin) => {
  let [activeRides, scheduledRides, openOffers, onlineDrivers, driverLocations, reports] =
    await Promise.all([
      admin.select("rides", {
        limit: 24,
        order: "created_at.desc",
...
export const getDriversData = async (admin, filters = {}, options = {}) => {
  ...
  const [rawVehicles, locations, wallets, payoutAccounts, payouts, recentRides] = await Promise.all([
    driverIds.length
      ? admin.select("vehicles", {
