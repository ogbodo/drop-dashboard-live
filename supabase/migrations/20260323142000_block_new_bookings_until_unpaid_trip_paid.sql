CREATE OR REPLACE FUNCTION public.assert_customer_has_no_unpaid_completed_trip(
  p_customer_id uuid
)
RETURNS void
LANGUAGE plpgsql
...
AS $$
DECLARE
  v_unpaid_ride_id uuid;
BEGIN
  SELECT r.id
  INTO v_unpaid_ride_id
  FROM public.rides r
  ...
  IF v_unpaid_ride_id IS NOT NULL THEN
    RAISE EXCEPTION
      'Please complete payment for your last trip before booking another ride or delivery.'
      USING ERRCODE = 'P0001',
            DETAIL = 'ride_id=' || v_unpaid_ride_id;
  END IF;
END;
$$;

GRANT EXECUTE ON FUNCTION public.assert_customer_has_no_unpaid_completed_trip(uuid)
